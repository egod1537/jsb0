...
ResolvedExecutionSpec Resolve(const SimScenario &scenario,
...
SimComparison::SimComparison(
    std::unique_ptr<SimRuntime> baseline,
...
SimComparison::~SimComparison() { Shutdown(); }

std::unique_ptr<SimComparison> SimComparison::Create(
...
std::unique_ptr<SimComparison> SimComparison::Create(
  ...
  if (!runtimeFactory) {
    runtimeFactory = [](const ResolvedExecutionSpec &execution,
      ...
      return SimRuntime::CreateForExecution(execution, factoryError);
    };
  }
  ...
  auto comparison = std::unique_ptr<SimComparison>(
      new SimComparison(std::move(baseline),
          std::move(primary),
...
bool SimComparison::IsRunning() const {
  return state_ == ComparisonExecutionState::Running;
}

bool SimComparison::IsFinished() const {
  return state_ == ComparisonExecutionState::Completed;
}

ComparisonExecutionState SimComparison::GetState() const {
  return state_;
}

double SimComparison::GetSimulationTimeSec() const {
  return std::min(durationSec_, static_cast<double>(stepCount_) * dtSec_);
}

std::uint64_t SimComparison::GetStepCount() const { return stepCount_; }

const std::string &SimComparison::GetLastError() const {
  return lastError_;
}

const ComparisonVariantResult &SimComparison::GetVariantResult(
    ExecutionVariant variant) const {
  return variant == ExecutionVariant::Baseline ? baselineResult_
                                               : primaryResult_;
}

ComparisonObservation SimComparison::TakeObservation() {
  return std::exchange(observation_, {});
}

bool SimComparison::CollectEvents() {
...
SimComparison::FindDivergedClockVariant() const {
...
bool SimComparison::Fail(ExecutionVariant variant, std::string error) {
