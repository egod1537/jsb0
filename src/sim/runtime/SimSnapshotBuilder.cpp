...
namespace sim {
SimInstanceSnapshot SimSnapshotBuilder::CaptureInstance(
...
AutopilotSnapshot SimSnapshotBuilder::CaptureAutopilot(
    const Simulation &simulation) {
  AutopilotSnapshot snapshot;
  const auto &manager = simulation.GetFlightControlManager();
...
LinearizationSnapshot SimSnapshotBuilder::CaptureLinearization(
    const Simulation &simulation) {
