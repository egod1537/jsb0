...
void Validate(const SimScenario &scenario) {
  ScenarioValidationError error;
  if (!ValidateSimScenario(scenario, &error)) {
    throw std::runtime_error(error.ToString());
  }
}

SimScenario ParseScenario(const YAML::Node &root,
...
std::string SimScenarioSerializer::Serialize(
...
bool SimScenarioSerializer::Deserialize(std::string_view yaml,
...
bool SimScenarioSerializer::Load(const std::filesystem::path &path,
    SimScenario &scenario, std::string &error,
    ScenarioLoadMetadata *metadata) {
...
bool SimScenarioSerializer::Save(const std::filesystem::path &path,
