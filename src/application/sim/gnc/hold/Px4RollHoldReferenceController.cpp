...
std::optional<double> Px4RollHoldReferenceController::OnTick(
  ...
  const double unscaledTorque =
      settings_.rateProportionalGain * rateError + rateIntegrator_
      - settings_.rateDerivativeGain * properties.P().DotRadPerSec2()
      + scaledFeedForward * rateSetpoint;
  ...
  UpdateIntegrator(rateError,
      dtSec,
      rawTorqueCommand > 1.0,
      rawTorqueCommand < -1.0);
