...
void Simulation::PublishAutopilotTelemetry(const sim::Tick &tick) {
  ...
  if (const auto *rollHold =
          controllers != nullptr
              ...
              : nullptr) {
    aileronCommand = rollHold->GetDiagnostics().aileronCommand;
  }
  if (const auto *px4RollReference =
          controllers != nullptr
              ...
              : nullptr) {
    aileronCommand = px4RollReference->GetDiagnostics().aileronCommand;
  }
  const double commandedRollRad = rollHoldCapability != nullptr
