...
namespace gui {
LinearizationWindow::LinearizationWindow()
...
void LinearizationWindow::OnRender(GUI &gui) {
  ...
  auto *analysis = dynamic_cast<gnc::IAutopilotAnalysis *>(
  ...
  bool automaticUpdates = analysis->IsAutomaticLinearizationEnabled();
  if (ImGui::Checkbox("Automatic linearization", &automaticUpdates)) {
    analysis->SetAutomaticLinearizationEnabled(automaticUpdates);
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
  ...
  const bool inProgress = analysis->IsLinearizationInProgress();
  const std::string_view errorMessage =
      analysis->GetLinearizationErrorMessage();
  if (!automaticUpdates) {
    ...
    ImGui::TextDisabled(analysis->GetLinearizationResult() != nullptr
  ...
  ImGui::Separator();
  const gnc::LinearizationResult *result = analysis->GetLinearizationResult();
  if (result == nullptr) {
  ...
  DrawResult(*result, inProgress, errorMessage);
}

void LinearizationWindow::DrawTransformSelector() {
  int selectedTransform = static_cast<int>(valueTransform_);
  ...
  if (valueTransform_ == LinearizationValueTransform::SignedLog10) {
    ImGui::SameLine();
...
void LinearizationWindow::DrawResult(const gnc::LinearizationResult &result,
  ...
  if (ImGui::BeginTabItem("Overview")) {
    DrawOverview(result, updateInProgress, errorMessage, valueTransform_);
    ImGui::EndTabItem();
  }

  if (ImGui::BeginTabItem("Dynamics")) {
    DrawDynamics(result, valueTransform_);
    ImGui::EndTabItem();
  }
  ...
  if (ImGui::BeginTabItem("Longitudinal")) {
    ...
    DrawSubsetMatrices("Longitudinal", result, states, inputs, valueTransform_);
    ImGui::EndTabItem();
  }
  ...
  if (ImGui::BeginTabItem("Lateral")) {
    ...
    DrawSubsetMatrices("Lateral", result, states, inputs, valueTransform_);
    ImGui::EndTabItem();
  }
