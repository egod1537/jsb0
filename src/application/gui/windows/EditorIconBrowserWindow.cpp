...
EditorIconBrowserWindow::EditorIconBrowserWindow() : Window("Editor Icons") {
  SetVisible(false);
}

void EditorIconBrowserWindow::OnRender(GUI &gui) {
  EditorIconRegistry &icons = gui.GetEditorIcons();
  const std::vector<EditorIconInfo> &iconIndex = icons.GetIndex();
  ...
  while (clipper.Step()) {
    for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; ++row) {
      ...
      const EditorIconHandle icon = icons.Get(info.relativeName);
      if (icon.IsValid()) {
