...
bool IsBaselineAutopilotAvailable(const sim::SimSnapshot &snapshot) {
...
const sim::AutopilotSnapshot *GetAutopilotSnapshot(
    const sim::SimSnapshot &snapshot, AutopilotSelection selection) {
...
const sim::SimInstanceSnapshot *GetInstanceSnapshot(
    const sim::SimSnapshot &snapshot, AutopilotSelection selection) {
  if (selection == AutopilotSelection::Baseline) {
...
GNCWindow::GNCWindow() : Window("GNC", editor_icon_aliases::GNC) {}

GNCWindow::GNCWindow(GNCController &controller) : GNCWindow() {
  controller_ = &controller;
}

void GNCWindow::OnRender(const sim::SimSnapshot &snapshot) {
              ...
              + ui::Tab("Autopilot")
                    [
                      ui::Custom([this,
                        ...
                        const sim::SimInstanceSnapshot *instance =
  ...
  controller_->OnEvent(ExperimentalViewStateChanged{
